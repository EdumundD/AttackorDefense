...
public class FrameInput
{
    ...
    public FrameInput(int tick,string data)
    {
        this.tick = tick;
        clientID = Int32.Parse(data.Split('/')[0]);
        buildTower = new BuildTower(data.Split('/')[1]);
        createBarrack = new CreateBarrack(data.Split('/')[2]);
        createSoldier = new CreateSoldier(data.Split('/')[3]);
        objectDeath = new ObjectDeath(data.Split('/')[4]);
    }
    //Default
    public FrameInput() {
        tick = -1;
        clientID = (int)GameFacade.Instance.URoleType;
